...
Copyright 2016  Stefan Senk  (email : info@senktec.com)
...
add_action('wp_footer', function(){
	...
	if( !empty($options['fgcolor']) ) {
		$styles .= 'color: ' . $options['fgcolor'] . ';';
	}
	$html = '<div id="icc_message" style="' . $styles . '">' . do_shortcode( stripslashes( $options['message'] ) ) . '</div>';
	echo apply_filters('icc_message_html', $html);
});

add_shortcode('icc_dismiss', function() {
	$options = get_option( 'implied_cookie_consent' );
	$dismiss_text = (empty($options['dismiss_text']) ? '&#9746;' : $options['dismiss_text']);
	$html = '<span class="icc_dismiss_button">' . $dismiss_text . '</span>';
