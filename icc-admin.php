...
global $implied_cookie_consent_db_version;
$implied_cookie_consent_db_version = '1.3';

function icc_get_options(){
...
<p><strong>This page should be completed by the website admin...</strong></p>', 'implied-cookie-consent' );
	...
	update_option( $opt_name, icc_get_options() );
...
add_action('admin_menu', function(){
	add_options_page(
		...
		$opt_fgcolor_name = 'icc_fgcolor';
		$opt_dismiss_text_name = 'icc_dismiss_text';

		$options = icc_get_options();

		if( isset($_POST[ $hidden_field_name ]) && $_POST[ $hidden_field_name ] == 'Y' ) {
			$options['message'] = stripslashes( $_POST[ $opt_message_name ] );
			$options['bgcolor'] = $_POST[ $opt_bgcolor_name ];
			$options['fgcolor'] = $_POST[ $opt_fgcolor_name ];
			$options['dismiss_text'] = $_POST[ $opt_dismiss_text_name ];
...
?>
		<div class="wrap">
			<?php screen_icon(); ?>
			<h1><?php _e( 'Implied Cookie Consent Options', 'implied-cookie-consent' ); ?></h1>

			<form name="icc_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
				<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">

				<h2>Message</h2>
				<p><label for="<?php echo $opt_message_name; ?>"><?php _e("Cookie info message displayed to the user (optionally use the [icc_dismiss] shortcode to add a dismiss button): ", 'implied-cookie-consent'); ?></label></p>
				<textarea name="<?php echo $opt_message_name; ?>" rows="3" class="large-text"><?php echo stripslashes( $options['message'] ); ?></textarea>

				<table class="form-table">
					<tr>
						<th scope="row"><label for="<?php echo $opt_dismiss_text_name; ?>"><?php _e("Dismiss text: ", 'implied-cookie-consent'); ?></label></th>
						<td><input type="text" name="<?php echo $opt_dismiss_text_name; ?>" value="<?php echo $options['dismiss_text']; ?>" class="regular-text">
						<p class="description"><?php _e("Text to be displayed on the dismiss link.", 'implied-cookie-consent'); ?></p></td>
					</tr>
					<tr>
						<th scope="row"><label for="<?php echo $opt_fgcolor_name; ?>"><?php _e("Text color: ", 'implied-cookie-consent'); ?></label></th>
						<td><input type="text" name="<?php echo $opt_fgcolor_name; ?>" value="<?php echo $options['fgcolor']; ?>" size="20" class="icc_color_field">
						<p class="description"><?php _e("Text color displayed for the cookie info message.", 'implied-cookie-consent'); ?></p></td>
					</tr>
					<tr>
						<th scope="row"><label for="<?php echo $opt_bgcolor_name; ?>"><?php _e("Background color: ", 'implied-cookie-consent'); ?></label></th>
						<td><input type="text" name="<?php echo $opt_bgcolor_name; ?>" value="<?php echo $options['bgcolor']; ?>" size="20" class="icc_color_field">
						<p class="description"><?php _e("Background color displayed for the cookie info message.", 'implied-cookie-consent'); ?></p></td>
					</tr>
				</table>
